...
function computeStats(data) {
  ...
  questions.forEach(q => {
    ...
    maxAttempts = Math.max(maxAttempts, q.attempts.length);

    if (q.attempts[0]?.correct) firstTryCorrect++;
    ...
    q.attempts.forEach(a => {
      ...
      answers[letters[a.userSolution[0]]]++;
    });

    if (solved) eventuallyCorrect++;
    else abandoned++;

    attemptsDistribution[q.attempts.length] =
      (attemptsDistribution[q.attempts.length] || 0) + 1;
  });
  ...
  return {
    ...
    firstTryAccuracy: ((firstTryCorrect / totalQuestions) * 100).toFixed(1),
    eventuallyCorrect,
    abandoned,
    avgAttempts: (totalAttempts / totalQuestions).toFixed(2),
    maxAttempts,
